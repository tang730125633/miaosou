...
- **网址粘贴直达**：支持系统 `⌘C / ⌘V / ⌘A`；粘贴 `https://...` 或 `www.` 地址后，回车固定用 Chrome 打开。
...
- **系统 Quick Look**：在 `.` 后缀模式或 `@` 隐藏配置模式中选中文件，按 `Space` 打开/关闭 macOS 原生预览；预览确认后可直接拖进聊天窗口。
...
> 构建脚本会优先用本机的 Apple Development 证书签名（这样「辅助功能」等授权在重编译后不失效），没有证书时回退临时签名。安装包尚未做 Apple 公证，其他 Mac 首次运行时可能需要右键点击 App，选择“打开”。SHA-256 校验文件随 Release 一起提供。
